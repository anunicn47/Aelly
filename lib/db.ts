const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠Aelly💠. I guess you were trying to use my commands in``` *{Aelly.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\nIf you are keen to use my features, you can deploy Aelly on your own account without much effort, in less than 5 minutes! Check out the links given below.```\n\n🔗https://myAelly.com\n🔗https://github.com/AellyOfficial/Aelly",
        ERROR_TEMPLATE:
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION:
            "```Get the profile picture of the group in a group conversation or the profile picture of Aelly itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by Aelly.```",

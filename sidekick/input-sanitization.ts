...
const getCleanedContact = async (args: string[], client: Client, Aelly: Aelly) => {
    var jidNumber = '';
    ...
    if (parseInt(args[0]) === NaN || args[0][0] === "+" || args[0][0] === "@") {
        if (args[0][0] === "@" || args[0][0] === "+") {
            ...
            client.sendMessage(Aelly.chatId,"*Enter valid contact number.* Approved Syntax:\n```1. XXXXXXXXXX``` \n```2. Tag the person``` \n```3. +(YYY)XXXXXXXXXX.``` \n_(YY- Country Code, without zeros)_", MessageType.text);
            return undefined;
        }
    } else {
        jidNumber = args[0];       
...
const handleError = async (err, client, Aelly, customMessage = "```Something went wrong. The error has been logged in log chats```") => {
    console.log(chalk.redBright.bold("[ERROR] " + err));
    let data = {
        commandName: Aelly.commandName,
        fromMe: Aelly.fromMe,
        isReply: Aelly.isReply,
        isGroup: Aelly.isGroup,
        isPm: Aelly.isPm,
        isImage: Aelly.isImage,
        isBotGroupAdmin: Aelly.isBotGroupAdmin,
        isSenderGroupAdmin: Aelly.isSenderGroupAdmin,
        isSenderSudo: Aelly.isSenderSUDO,
        err: err
    }
    client.sendMessage(Aelly.chatId, customMessage, MessageType.text);
    client.sendMessage(Aelly.logGroup, format(ERROR_TEMPLATE, data), MessageType.text);
}

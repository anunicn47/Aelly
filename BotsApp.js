...
async function main() {
    ...
    client.on('chat-update', async chat => {
        ...
        var Aelly = wa.resolve(chat, client, groupMetadata);
        // console.log(Aelly);
        if (Aelly.isCmd) {
            let isBlacklist = await Blacklist.getBlacklistUser(Aelly.sender, Aelly.chatId);
            const cleared = await clearance(Aelly, client, isBlacklist);
            if (!cleared) {
                return;
            }
            ...
            try {
                command.handle(client, chat, Aelly, args).catch(err => console.log("[ERROR] " + err));
            } catch (err) {
                console.log(chalk.red("[ERROR] ", err));
            }
        }

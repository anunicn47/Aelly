...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, Aelly: Aelly, args: string[]): Promise<void> {
                ...
                qrcode.callback = async function (err: string | string[], value: { result: any; }) {
                    if (err) {
                        ...
                        if (err.includes('enough finder patterns')) {
                            await client
                                .sendMessage(Aelly.chatId, Strings.decodeqr.INVALID_INPUT, MessageType.text);
                        }
                    } else {
                        await client
                            .sendMessage(Aelly.chatId, value.result, MessageType.text)
                            .catch((err) =>
        ...
        try {
            if (!Aelly.isTextReply || (Aelly.isReplyAudio || Aelly.isReplyVideo || Aelly.isReplyAnimatedSticker)) {

                await client
                    .sendMessage(Aelly.chatId, DECODE.INVALID_REPLY, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, Aelly));
                return;

            } else if (Aelly.isReplySticker) {

                processing = await client
                    .sendMessage(Aelly.chatId, DECODE.PROCESSING, MessageType.text);
                var replyChatObject = {
            ...
            } else if (!Aelly.isImage) {

                await client
                    .sendMessage(Aelly.chatId, DECODE.INVALID_INPUT, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, Aelly));
                return;

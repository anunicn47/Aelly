...
module.exports = {
    ...
    async handle(
        ...
        if (args.length === 0) {
            await client
                .sendMessage(Aelly.chatId, NEWS.NO_COMMMAND, MessageType.text)
        ...
        if (args[0] == "search") {
            ...
            await client
                .sendMessage(Aelly.chatId, message, MessageType.text)
                .catch((err) => inputSanitization.handleError(err, client, Aelly));
            return;
        }
        if (args[0] == "fetch") {
            ...
            if (!searchTerm) {
                await client
                    .sendMessage(Aelly.chatId, NEWS.NO_PUB_NAME, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, Aelly));
                return;
            }
            let searchResponse;
            ...
            let message =
                ...
                foundPub +
                "* ```is being fetched by Aelly, this may take some time, please be patient!```";
            if (!foundPub) {
                message = "```Sorry, no publication found by that name!```";
                await client
                    .sendMessage(Aelly.chatId, message, MessageType.text)
            ...
            await client
                ...
                .catch((err) => inputSanitization.handleError(err, client, Aelly));
            message = "```Your requested publication fetched by Aelly``` ☝️";
            await client
                .sendMessage(Aelly.chatId, message, MessageType.text)
                .catch((err) => inputSanitization.handleError(err, client, Aelly));
            return;

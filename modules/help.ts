...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, Aelly: Aelly, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            ...
            if(!args[0]){
                ...
                client.sendMessage(Aelly.chatId, helpMessage, MessageType.text).catch(err => inputSanitization.handleError(err, client, Aelly));
                return;
            }
            ...
            if(command){
                ...
                if(command.demo?.isEnabled) {
                    ...
                    await client.sendMessage(Aelly.chatId, buttonMessage, MessageType.buttonsMessage).catch(err => inputSanitization.handleError(err, client, Aelly))
                    return;
                }
            ...
            client.sendMessage(Aelly.chatId, HELP.COMMAND_INTERFACE + "```Invalid Command. Check the correct name from```  *.help*  ```command list.```", MessageType.text).catch(err => inputSanitization.handleError(err, client, Aelly));
        } catch (err) {
